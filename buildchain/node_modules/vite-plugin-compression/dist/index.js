...
${import_chalk.default.cyan("\u2728 [vite-plugin-compression]:algorithm=" + algorithm)} - compressed file successfully: `);
  ...
  compressMap.forEach((value, name) => {
    ...
    const rName = (0, import_vite.normalizePath)(cname).replace((0, import_vite.normalizePath)(`${config.build.outDir}/`), "");
    const sizeStr = `${oldSize.toFixed(2)}kb / ${algorithm}: ${size.toFixed(2)}kb`;
    config.logger.info(import_chalk.default.dim(import_path2.default.basename(config.build.outDir) + "/") + import_chalk.default.blueBright(rName) + " ".repeat(2 + maxKeyLength - name.length) + " " + import_chalk.default.dim(sizeStr));
  });
  config.logger.info("\n");
}
